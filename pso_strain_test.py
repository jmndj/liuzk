...
def Coordinate(Plane,Crystallographic,a):
    ...
    y2=-o2/o4
    z2=-o3/o4
    return(x1,y1,z1),(x2,y2,z2),(x3,y3,z3),(theta1,fai1,beta1)
...
###########################################
    ...
    #         time.sleep(10)
     if (os.path.exists('tmp.dat')):     
         ...
         df = pd.read_csv('./tmp.csv', names=['strain', 'stress'])
...
class Particle:                     # 定义一个类
    def __init__(self,x0):
        ...
        for i in range(0,num_dimensions):
            self.velocity_i.append(random.uniform(-1,1))     #random.uniform随机生成一个浮点数，
    ...
    def evaluate(self,costFunc):                             #定义评估函数
        ...
        if self.err_i < self.err_best_i or self.err_best_i==0:
...
class PSO():                                                  #定义一个类
	...
	#将每个粒子初始化                    
        swarm[0].position_i=[1,3,1]
        swarm[1].position_i=[0,1,1]
        swarm[2].position_i=[1,1,1]
        swarm[3].position_i=[0,1,2]
        swarm[4].position_i=[1,1,2]
        swarm[5].position_i=[2,2,1]
        swarm[6].position_i=[2,1,3]
        swarm[7].position_i=[1,2,3]
        ...
        while i < maxiter:
            ...
            for j in range(0,num_particles):                     #遍历每个粒子
                ...
                sys.stdout = file
                print(i,swarm[j].pos_i,err_real_i) # 输出个体最优方向和最优值
                sys.stdout.close()
        ...
        temp = sys.stdout
        file = open('psoresult_compress','w')
        sys.stdout = file
        print('FINAL:')
        print(pos_best_g)
...
PSO(func1,initial,bounds,num_particles=8,maxiter=10)
